...
public class playerHP : MonoBehaviour
{
    ...
    {
        Instantiate(PlayerDeath, transform.position, Quaternion.Euler(new Vector3(-90, 0, 0)));
        damageScreen.color = flashColor;
        Destroy(gameObject);
        Animator endGameAnim = endGameText.GetComponent<Animator>();
        endGameAnim.SetTrigger("endGame");
        Animator contiAnim = endGameButton.GetComponent<Animator>();
        contiAnim.SetTrigger("conti");
    }
}
